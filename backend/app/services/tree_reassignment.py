...
async def get_trees_needing_assignment(
    ...
    trees = db.query(InventoryTree).filter(
        InventoryTree.forest_block_id == block_id
    ).all()
    ...
    for tree in trees:
        ...
        tree_data.append({
            ...
            "suggested_compartment_id": str(suggested_comp.id) if suggested_comp else None,
            "suggested_compartment_name": suggested_comp.compartment_code if suggested_comp else None
        })
...
async def auto_assign_trees_by_location(
    ...
    for tree in trees:
        ...
        if compartment:
            # Assign tree to compartment
            tree.compartment_id = compartment.id
            tree.forest_block_id = None  # Remove block assignment
            assigned_count += 1
...
async def manual_assign_trees(
    ...
    Args:
        db: Database session
        manual_assignments: Dict mapping tree_id -> compartment_id
    ...
    for tree_id, compartment_id in manual_assignments.items():
        try:
            # Get tree
