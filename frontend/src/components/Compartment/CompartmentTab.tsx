...
interface CompartmentTabProps {
  ...
  parentBlocks?: Array<{ block_id: string; block_name: string }>;
}

export function CompartmentTab({ calculationId, parentBlocks }: CompartmentTabProps) {
  ...
  const loadCompartmentTree = async () => {
    try {
      ...
      const tree = data.blocks || [];
      if (parentBlocks && parentBlocks.length > 0) {
        const nameMap = new Map(parentBlocks.map((p: any) => [p.block_id, p.block_name]));
        const renameBlock = (node: any): any => {
          if (node.is_compartment || node.division_level > 0) return node;
          if (nameMap.has(node.id)) return { ...node, name: nameMap.get(node.id) };
          return node;
        };
        setCompartmentTree(tree.map(renameBlock));
      } else {
  ...
  const loadBlocks = useCallback(async () => {
    try {
      ...
      let data = await compartmentApi.getAvailableBlocks(calculationId);
      if (parentBlocks && parentBlocks.length > 0) {
        data = data.map((b: AvailableBlock) => {
          const match = parentBlocks.find(p => p.block_id === b.id);
          return match ? { ...b, name: match.block_name } : b;
        });
      }
  ...
  }, [calculationId, selectedBlock?.id, parentBlocks]);
