...
const MapEditor: React.FC<MapEditorProps> = ({
  ...
  useEffect(() => {
    ...
    // Render blocks in all modes (needed as reference for sub-area drawing)
    if (mode !== 'edit_boundary' && mode !== 'edit_blocks' && mode !== 'edit_subareas') {
      console.log('[MapEditor] Skipping block render - unknown mode');
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const results: { saved: number; failed: number; messages: string[] } = { saved: 0, failed: 0, messages: [] };
        ...
        if (savedIds.has(subArea.id)) {
          ...
          if (nameChanged || categoryChanged || geometryChanged) {
            ...
            try {
              ...
              results.failed++;
              results.messages.push(`"${subArea.name}": ${errorMsg}`);
            }
          } else {
            results.saved++;
          }
      ...
      if (results.failed > 0) {
        setError(
          `Saved ${results.saved} sub-area${results.saved !== 1 ? 's' : ''}, ` +
          `skipped ${results.failed}:\n${results.messages.join('\n')}`
        );
      } else if (results.saved > 0) {
        console.log(`All ${results.saved} sub-area${results.saved !== 1 ? 's' : ''} saved successfully`);
      }
