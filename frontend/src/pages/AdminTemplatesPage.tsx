...
import { Tabs, List, Tag, Typography, Spin, Empty, Button, message, Modal, Input, Space, Alert, Descriptions, Tooltip } from 'antd';
import { CheckCircleOutlined, CloseCircleOutlined, ClockCircleOutlined, EyeOutlined, SendOutlined, TagsOutlined } from '@ant-design/icons';
import { operationalPlanApi } from '../services/api';
import TemplateCategoryManager from '../components/OperationalPlan/TemplateCategoryManager';
    ...
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <Tabs
        ...
        items={[
          {
            key: 'approvals',
            label: <span><ClockCircleOutlined /> Pending Approvals</span>,
            children: (
              <>
                      ...
                      <List.Item
                        actions={[
                          <Tooltip key="approve" title="Approve as global template">
                            <Button type="primary" icon={<CheckCircleOutlined />} onClick={() => openAction(tmpl, 'approve')}>Approve</Button>
                          </Tooltip>,
                          <Tooltip key="reject" title="Reject with note">
                            <Button danger icon={<CloseCircleOutlined />} onClick={() => openAction(tmpl, 'reject')}>Reject</Button>
                          </Tooltip>,
                        ]}
                      >
                        <List.Item.Meta
                          ...
                          description={
                            <div>
                              ...
                              <div style={{ marginTop: 4 }}>{tmpl.tags?.map(t => <Tag key={t} style={{ fontSize: 11 }}>{t}</Tag>)}</div>
                ...
                >
                  <div style={{ marginBottom: 12 }}><Text strong>{actionModal.template?.name}</Text></div>
                  {actionModal.action === 'reject' && (
                    <div style={{ marginBottom: 8 }}><Text type="danger">Provide a reason for rejection (required):</Text></div>
                  )}
                  <TextArea value={note} onChange={(e) => setNote(e.target.value)} rows={3} placeholder={actionModal.action === 'approve' ? 'Optional approval note' : 'Reason for rejection...'} />
                </Modal>
              </>
            ),
          },
          {
            key: 'categories',
            label: <span><TagsOutlined /> Categories</span>,
            children: <TemplateCategoryManager />,
          },
        ]}
      />
    </div>
