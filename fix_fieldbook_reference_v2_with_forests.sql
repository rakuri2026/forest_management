-- COMPLETE VERSION: Includes ALL important feature types for fieldbook references
-- Added: ESA Forest Boundary + Community Forests
...
BEGIN
    ...
    -- Filter within 100m FIRST using ST_DWithin (uses spatial indexes!)
    ...
    WITH nearby_features AS (
        ...
        -- ESA Forest Boundaries (NEW)
        SELECT
            COALESCE(description, "boundary of") as name,
            ST_Distance(v_point_utm, ST_Transform(geom, 32645)) as distance_m,
            degrees(ST_Azimuth(v_point, ST_ClosestPoint(geom, v_point))) as azimuth
        FROM admin."esa_forest_Boundary"
        WHERE (description IS NOT NULL OR "boundary of" IS NOT NULL)
          AND ST_DWithin(ST_Transform(geom, 32645), v_point_utm, 100)

        UNION ALL

        -- Community Forests (NEW - VERY IMPORTANT)
        SELECT
            name,
            ST_Distance(v_point_utm, ST_Transform(geom, 32645)) as distance_m,
            degrees(ST_Azimuth(v_point, ST_ClosestPoint(geom, v_point))) as azimuth
